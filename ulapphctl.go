...
// COPYRIGHT (c) 2017-2020 Edwin D. Vinas, Opensource Version, Dockerized
/////////////////////////////////////////////////////////////////////////////////////////////////
//REV ID: 	D0001
...
/////////////////////////////////////////////////////////////////////////////////////////////////
//REV ID: 	D0007
//REV DATE: 2020-Jan-27
//REV DESC:	Log datastore events 
//REV AUTH:	Edwin D. Vinas
/////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////////////
...
func main() {
	...
	app.Commands = []cli.Command{
	{
	  ...
	  Action:  func(c *cli.Context) error {
		if config == "" {
			log.Printf("ERROR: Missing configuration file!")
	...
	},
	{
	  ...
	  Action:  func(c *cli.Context) error {
		if account == "" {
			log.Printf("ERROR: Missing Google account parameter")
			log.Printf("\nTry: ulapphctl deploy --project your-ulapph-cloud-desktop --account demo.ulapph@gmail.com --yaml app.yaml\n")
			return nil
		}
		if project == "" {
			log.Printf("ERROR: Missing Project ID parameter")
			log.Printf("\nTry: ulapphctl deploy --project your-ulapph-cloud-desktop --account demo.ulapph@gmail.com --yaml app.yaml\n")
			return nil
		}
		if yaml == "" {
			log.Printf("ERROR: Missing YAML parameter")
			log.Printf("\nTry: ulapphctl deploy --project your-ulapph-cloud-desktop --account demo.ulapph@gmail.com --yaml app.yaml\n")
			return nil
		}
...
func configureUlapphCloudDesktop(CFG_FILE string) (err error) {
	...
	log.Printf("app_id: %#v", Config.Project[0].Appid)
	...
	for scanner.Scan() {
		FL_WRITTEN_OK := false
			...
			if len(tLineStr2) > 2 && string(tLineStr2[0]) != "/" && string(tLineStr2[1]) != "/" {
				...
				i = strings.Index(sLineText, "var xAssistantWorkspace")
				if i != -1 {
					//-----------------------------
					//Configuring installation
					//log.Printf("\n++ Processing var xAssistantWorkspace...  ")
					//replace
					configValue := getFromConfig("var xAssistantWorkspace")
					i = strings.Index(configValue, "map[")
					if i != -1 {
						buf.WriteString(fmt.Sprintf("    %v = %v\n", "var xAssistantWorkspace", configValue))
						//log.Printf("\nNEWLINE014: %v", fmt.Sprintf("    %v = %v\n", "var xAssistantWorkspace", configValue))
					} else {
						buf.WriteString(fmt.Sprintf("var xAssistantWorkspace = `%v`\n", configValue))
						//log.Printf("\nNEWLINE015: %v", fmt.Sprintf("var xAssistantWorkspace = `%v`\n", configValue))
					}
				...
				/*i = strings.Index(sLineText, "var uwmWallpapers")
				if i != -1 {
					//-----------------------------
					//Configuring installation
					//log.Printf("\n++ Processing var uwmWallpapers...  ")
					//replace
					configValue := getFromConfig("var uwmWallpapers")
					i = strings.Index(configValue, "map[")
					if i != -1 {
						buf.WriteString(fmt.Sprintf("    %v = %v\n", "var uwmWallpapers", configValue))
						//log.Printf("\nNEWLINE014: %v", fmt.Sprintf("    %v = %v\n", "var uwmWallpapers", configValue))
					} else {
						buf.WriteString(fmt.Sprintf("var uwmWallpapers = `%v`\n", configValue))
						//log.Printf("\nNEWLINE015: %v", fmt.Sprintf("var uwmWallpapers = `%v`\n", configValue))
					}
                ...
                //D0007
                i = strings.Index(sLineText, "datastoreClient.Get() [")
                if i != -1 {
					//ERR1CTR = ERR1CTR + 1
                    ERRCODE := padNumberWithZero(4, lineCtr)
                    SPL := strings.Split(sLineText, "[")
                    sLineTextNew := fmt.Sprintf("%v [%v]\")", SPL[0], ERRCODE)
                    //sLineTextNew := strings.Replace(sLineText, "[PUT_LINE_HERE]", fmt.Sprintf("[%v]", ERRCODE), -1)
					buf.WriteString(fmt.Sprintf("%v\n", sLineTextNew))
					FL_WRITTEN_OK = true
                }
                i = strings.Index(sLineText, "datastoreClient.Put() [")
                if i != -1 {
					//ERR1CTR = ERR1CTR + 1
                    ERRCODE := padNumberWithZero(4, lineCtr)
                    SPL := strings.Split(sLineText, "[")
